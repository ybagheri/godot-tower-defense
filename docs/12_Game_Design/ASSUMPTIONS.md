...
| A7 | Map visuals (Line2D route) and stage route waypoints are synchronized MANUALLY in M4; a path-drawing editor tool is the future fix | No tooling milestone yet | CLOSED 2026-08-28 by SPEC-0016: routes live in resources/paths/*.tres and the route_id meta binding is enforced by content_validation_test, so drift fails CI instead of shipping |
